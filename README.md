# About Me 👨‍💻
## 더 높은 기술 수준을 목표로 하고 있는 고등학생 홍성진입니다
<a href="https://devgarlic.github.io/" target="_blank"><img src="https://img.shields.io/badge/BLOG-000000?style=flat&logo=GitHub&logoColor=white"/></a>
<a href="mailto:DevGarlic@proton.me" target="_blank"><img src="https://img.shields.io/badge/MAIL-D14836?style=flat&logo=Mail.Ru&logoColor=white"/></a>
<a href="https://www.instagram.com/hsj._.08" target="_blank"><img src="https://img.shields.io/badge/INSTAGRAM-bc4a7b?style=flat&logo=Instagram&logoColor=white"/></a>
<a href="https://discord.com/users/1190688736632119477" target="_blank"><img src="https://img.shields.io/badge/Discord-5865f2?style=flat&logo=Discord&logoColor=white"/></a>   
<a href="https://www.buymeacoffee.com/Garlic" target="_blank"><img src="https://img.shields.io/badge/COFFEE-ffdd00?style=flat&logo=buymeacoffee&logoColor=white"/></a><br>
![DevGarlic's GitHub stats](https://github-readme-stats.vercel.app/api?username=DevGarlic&show_icons=true&theme=radical)

초등학교 3학년 때부터 스크래치와 엔트리와 같은 시각적 프로그래밍 환경을 통해 코딩에 흥미를 느끼기 시작했고,   
초등학교 5학년때부터 때부터 파이썬과 같은 실제 프로그래밍 언어를 배우며 본격적으로 개발을 시작했습니다.    
이를 통해 프로그래밍의 기초를 탄탄히 다지고, 스스로 학습하며 능력을 향상시켰습니다.   

중학생이 되고난 후 Flask와 같은 실제 백엔드를 구현해보면서 기술 수준을 향상시켰고,   
기회가 될때마다 코딩관련 대회에 나가며 수상경력을 쌓았습니다.   

# 기술 스택 💻
## Python 🐍
프로그램이나 백엔드 등 실제 수익을 창출할 수 있는 서비스를 구현할 수 있는 수준입니다.   
가장 선호하는 언어로 주로 파이썬을 통해 개발하고 웹개발과 관련된 콘텐츠에 흥미가 있습니다.   

## JavaScript 🟡
공부하고 있는 중에 있습니다✍

## HTML/CSS 🎨
기본적인 코드 작성, 기능 구현은 가능하나 퀄리티 높은 작품이나 디자인등은 아직 구현 불가능합니다.   

### 수상경력 🏆️
